...
@section('content_header')
	...
	<div class="pull-right">
		<a href="{{ url('admin/marcas') }}" style="display: inline;">
			<button type="button" class="btn btn-default btn-sm">Volver</button></a>
		</a>
		<a href="{{ url('admin/marcas/editar/'.$marca->id) }}" class="d-inline">
			<button type="button" class="btn btn-success btn-sm">Editar</button></a>
		</a>
		<!--Eliminar-->
		<form method="post" action="{{ url('admin/marcas/eliminar/'.$marca->id)}}" class="d-inline" onsubmit="return confirm('Confirma que desea eliminar el elemento?');">
			@csrf
...
@section('content')
	...
	<div class="form-group">
		...
		<p id="nombre">{{ $marca->nombre }}</p>
	</div>
	<div class="form-group">
		<label for="creacion">Fecha de Creacion</label>
		<p id="creacion">{{ $marca->created_at }}</p>
	</div>
	<div>
		<br>	
	</div>
