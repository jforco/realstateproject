...
@section('content_header')
	...
	<div class="pull-right">
		<a href="{{ url('admin/agentes') }}" style="display: inline;">
			<button type="button" class="btn btn-default btn-sm">Volver</button></a>
		</a>
		<a href="{{ url('admin/agentes/editar/'.$user->id) }}" style="display: inline;">
			<button type="button" class="btn btn-success btn-sm">Editar</button></a>
		</a>
		<!--Eliminar-->
		<form method="post" action="{{ url('admin/agentes/eliminar/'.$user->id)}}" style="display: inline;" onsubmit="return confirm('Confirma que desea eliminar el elemento?');">
			@csrf
